...
* File Name          : lcd_3029C.c
* Author             :  
* Version            :  
* Date               : 02/27/2014
* Description        : This file provides all the lcd 3029c EMI firmware functions.
***************************************************************************/
//===================================================================
//                                                     	            |
//                       INCLUDE FILES                              |
...
const struct emimode emi_3029C ={
	.red_width = 5,
	.green_width = 6,
	.blue_width = 5,
...
{
lcd_write_cmd_8bit(0x11); // exit sleep
	Delay_MS(120);
...
*/
	lcd_write_cmd_8bit(0xcc);//tiao wen   max 3f
	lcd_write_parameter_8bit(0x19); //vcm 0a////cc-1c_cd-17    cc-1b_cd-1f
	lcd_write_cmd_8bit(0xcd);//se cai
	lcd_write_parameter_8bit(0x1f); //vdv  1d max 1f
	lcd_write_cmd_8bit(0xce);
	lcd_write_parameter_8bit(0x00); //vcm1
...
void lcd_init(void) 
{
	deg_Printf("lcd_init mcu 3029C \n");
	blcd_FrameEnd_Flag = 0;	
	blcd_writeback_Flag = 0;

	PIN_CONF()

	lcd_emi_timing_init(emi_3029C);
...
u32 lcd_get_w()
{
	return emi_3029C.pwidth+1;
}

u32 lcd_get_h()
{
	return emi_3029C.pheight+1;
}
