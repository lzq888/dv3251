...
static void lcd_emi_init()
{
	...
	lcd_write_cmd_8bit(0xcc);
	lcd_write_parameter_8bit(0x0a); //vcm
	lcd_write_cmd_8bit(0xcd);
	lcd_write_parameter_8bit(0x1d); //vdv
...
void lcd_init(void) 
{
	deg_Printf("lcd_init mcu 3029 \n");
	blcd_FrameEnd_Flag = 0;	
	blcd_writeback_Flag = 0;

	PIN_CONF()

	lcd_emi_timing_init(emi_3029);
...
u32 lcd_get_w()
{
	return emi_3029.pwidth+1;
}

u32 lcd_get_h()
{
	return emi_3029.pheight+1;
}
