/* Filename:     hold_mode.c
 * Version:      1.0
 * Author:       zhiyong_zeng <zhiyong_zeng@buildwin.com.cn>
 * Started:      10th March 2014
 * Target Soc:   arm(32bit)
 * 
 * Description: Foring Hunter Soc to enter hold mode, this time, the system clock is gated, 
 * and clock to system logic modules is halted. No clock switching while entering the system logic
 * moudules so that power consumption is minimized due to no AC switching. However, the main clock
 * source are not disabled and they are still running, this allows the clock to be resumed in real
 * time. Importantly, the analog peripherals should be closed manually, and restored manually 
 * while system wake up.
 * 
 * Note: During hold mode, the device can be wake up by watchdog interrupt, rtcc interrupt, port
 * wakeup interrupt, all reset source.
 * when wake up from hold mode, if interrupt is enabled, hunter will enters corresponding interrupt
 * service subroutine(ISR), otherwise, hunter will directly execute the instruction following hold,
 * especially, if watchdog reset is enabled,, hunter will be reset directly.
 * 
...
//	__asm__("str	r2, [r3, #176]");	// 0xb0
	 ...
	 * 处理完中断后才继续跑下一条指令.
	 * */
	REG32(PCON0) |= (1 << CPU_HOLD);
//	__asm__("ldr	r2, [r3]");
//	__asm__("orr	r2, r2, #2");	// (1 << CPU_HOLD)
//	__asm__("str	r2, [r3]");
...
void cpu_hold_mode(void) {
	...
	sdram_code = (unsigned char const *) ((unsigned int) cpu_hold_func);
	sram_code = funpt;
